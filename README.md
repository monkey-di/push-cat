...
├── shared/                         # общие .md-фрагменты для include, вне синка
...
Чтобы не дублировать один и тот же кусок в нескольких скиллах (например, общую преамбулу под разные ролевые маски), есть build-time склейка. В любом `.md` ставится маркер:

```markdown
<!-- include: preamble -->
```

При синхронизации `push-cat.mjs` заменяет его содержимым `shared/preamble.md` (`.md` дописывается автоматически). Источник остаётся DRY — правишь фрагмент в одном файле; а в `~/.claude` и в публикуемые плагины попадает уже собранный файл, без маркера и без рантайм-чтений. Если файла во `shared/` нет — выводится предупреждение, маркер остаётся на месте.

Работает во всех `.md`: плагинных и standalone скиллах/командах, а также в `CLAUDE-GLOBAL.md`. Сам каталог `shared/` не публикуется и не синкается — это только сырьё для подстановки.
